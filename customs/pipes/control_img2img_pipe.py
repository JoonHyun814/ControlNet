...
class ControlNet_img2img_Pipe(StableDiffusionControlNetPipeline):
    ...
    def prepare_latents_noimg(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
    ...
    def __call__(
        ...
        prompt_embeds = prompt_embeds.type(torch.FloatTensor).to(device)
        ...
        if image:
            ...
            latents = self.prepare_latents(image, latent_timestep, batch_size, num_images_per_prompt, prompt_embeds.dtype, device, generator=None)
        else:
            num_channels_latents = self.unet.in_channels
            latents = self.prepare_latents_noimg(
